...
function makePackagePath(): string {
    ...
    const packagePath = path.resolve(tmpdir(), `${packageName}-${packageVersion}.vsix`);
...
async function installExtension(executablePath: string, packagePath: string, args: string[]) {

    const result = spawnSync(executablePath, ["--install-extension", packagePath, ...args]);
